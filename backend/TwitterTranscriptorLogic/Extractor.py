...
class Extractor:
    ...
    def getCaption(self, url):
            ...
            for message in raw_data['messages']:
                ...
                if "final" in body:
                    if body['final']:
                        ...
                        self.res.append(body['username']+':  '+body['body'])
            try:
        ...
        for linea in self.res:
            with open("ConversacionIngles.txt", "a") as f:
                f.write(linea + '\n')
        for linea in self.res:
            translator = Translator()
            result = translator.translate(linea, dest='es')
            with open("ConversacionEspañol.txt", "a", encoding="UTF-8") as f:
                try:
                    f.write(result.text + '\n')
                except:
                    print("Hay algo vacio")
